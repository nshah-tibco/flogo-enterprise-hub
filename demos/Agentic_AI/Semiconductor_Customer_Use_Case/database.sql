-- =====================================================================
-- Semiconductor Customer & Order Assistant — Agentic AI Use Case
-- PostgreSQL 14+   |   Database: semiconductor   |   Currency: USD
--
-- Vendor-neutral demo for the semiconductor components vertical.
-- Persona: a design engineer / procurement buyer at a customer company
-- (OEM, automotive tier-1, distributor) who self-serves over chat.
--
-- Part numbers are fictional-but-realistic MPNs plus industry-standard
-- JEDEC/Pro-Electron parts (74LVC1G08, BC847...) that any manufacturer
-- makes — nothing is tied to a specific brand, so the demo is reusable
-- across semiconductor verticals.
-- =====================================================================

DROP TABLE IF EXISTS alert_subscriptions CASCADE;
DROP TABLE IF EXISTS sample_requests     CASCADE;
DROP TABLE IF EXISTS rma_cases           CASCADE;
DROP TABLE IF EXISTS cross_references    CASCADE;
DROP TABLE IF EXISTS pcn_notices         CASCADE;
DROP TABLE IF EXISTS order_lines         CASCADE;
DROP TABLE IF EXISTS orders              CASCADE;
DROP TABLE IF EXISTS price_breaks        CASCADE;
DROP TABLE IF EXISTS inventory           CASCADE;
DROP TABLE IF EXISTS products            CASCADE;
DROP TABLE IF EXISTS customers           CASCADE;

-- ---------------------------------------------------------------------
-- customers  (master record, keyed by the account id the buyer uses)
-- ---------------------------------------------------------------------
CREATE TABLE customers (
    customer_id    VARCHAR(20)  PRIMARY KEY,          -- CUST-100xx
    company_name   VARCHAR(120) NOT NULL,
    contact_name   VARCHAR(80)  NOT NULL,
    email          VARCHAR(160) NOT NULL,
    phone          VARCHAR(40),
    region         VARCHAR(20)  NOT NULL,             -- EMEA / APAC / Americas
    industry       VARCHAR(40)  NOT NULL,             -- Automotive / Industrial / Consumer / Medical / Energy
    account_tier   VARCHAR(20)  NOT NULL CHECK (account_tier IN ('Distributor','Direct-Gold','Direct-Silver','Direct-Bronze')),
    currency       CHAR(3)      NOT NULL DEFAULT 'USD',
    credit_status  VARCHAR(10)  NOT NULL DEFAULT 'Good' CHECK (credit_status IN ('Good','Hold'))
);

-- ---------------------------------------------------------------------
-- products  (MPN catalog: specs, qualification, lifecycle)
-- ---------------------------------------------------------------------
CREATE TABLE products (
    part_number          VARCHAR(40) PRIMARY KEY,     -- manufacturer part number
    description          VARCHAR(160) NOT NULL,
    category             VARCHAR(40)  NOT NULL,        -- MOSFET / Schottky Rectifier / ESD Protection / Logic Gate / Bipolar Transistor / GaN FET
    package              VARCHAR(30)  NOT NULL,
    automotive_qualified BOOLEAN      NOT NULL DEFAULT FALSE,
    aec_qualification    VARCHAR(20),                  -- AEC-Q101 / AEC-Q100 / NULL
    rohs_compliant       BOOLEAN      NOT NULL DEFAULT TRUE,
    lifecycle_status     VARCHAR(10)  NOT NULL CHECK (lifecycle_status IN ('Active','NRND','EOL')),
    moq                  INTEGER      NOT NULL DEFAULT 1,   -- minimum order quantity
    unit_price_ref       NUMERIC(10,4) NOT NULL,           -- reference list price (detailed tiers in price_breaks)
    datasheet_url        VARCHAR(200)
);

-- ---------------------------------------------------------------------
-- inventory  (stock + lead time per distribution centre)
-- ---------------------------------------------------------------------
CREATE TABLE inventory (
    inventory_id     SERIAL PRIMARY KEY,
    part_number      VARCHAR(40) NOT NULL REFERENCES products(part_number),
    warehouse        VARCHAR(40) NOT NULL,             -- Hong Kong DC / Hamburg DC / Dallas DC
...
-- ---------------------------------------------------------------------
-- price_breaks  (quantity price tiers, USD)
-- ---------------------------------------------------------------------
CREATE TABLE price_breaks (
    ...
    currency       CHAR(3)     NOT NULL DEFAULT 'USD',
    UNIQUE (part_number, min_qty)
);

-- ---------------------------------------------------------------------
-- orders  +  order_lines  (existing orders; also the place/expedite target)
-- ---------------------------------------------------------------------
CREATE TABLE orders (
    order_id     VARCHAR(20) PRIMARY KEY,              -- SO-2026-NNNN
    customer_id  VARCHAR(20) NOT NULL REFERENCES customers(customer_id),
    po_number    VARCHAR(40),                          -- customer purchase order
    order_date   DATE        NOT NULL DEFAULT CURRENT_DATE,
    status       VARCHAR(20) NOT NULL CHECK (status IN ('Open','Confirmed','In Production','Shipped','Delivered','Cancelled')),
    currency     CHAR(3)     NOT NULL DEFAULT 'USD',
...
CREATE TABLE order_lines (
    ...
    requested_date DATE,
    promised_date  DATE,
    line_status    VARCHAR(20) NOT NULL DEFAULT 'Open' CHECK (line_status IN ('Open','Confirmed','In Production','Shipped','Delivered'))
);

-- ---------------------------------------------------------------------
-- pcn_notices  (Product/Process Change, Discontinuation, End-of-Life)
-- ---------------------------------------------------------------------
CREATE TABLE pcn_notices (
    pcn_id             VARCHAR(20) PRIMARY KEY,        -- PCN-2026-NNNN
    part_number        VARCHAR(40) NOT NULL REFERENCES products(part_number),
    notice_type        VARCHAR(10) NOT NULL CHECK (notice_type IN ('PCN','PDN','EOL')),
    title              VARCHAR(160) NOT NULL,
    description        TEXT,
    effective_date     DATE,
    last_time_buy_date DATE,
    replacement_part   VARCHAR(40),
    status             VARCHAR(15) NOT NULL DEFAULT 'Published'
);

-- ---------------------------------------------------------------------
-- cross_references  (alternates / second-source / replacements)
-- ---------------------------------------------------------------------
CREATE TABLE cross_references (
    xref_id          SERIAL PRIMARY KEY,
    part_number      VARCHAR(40) NOT NULL REFERENCES products(part_number),
    alternative_part VARCHAR(40) NOT NULL,
    xref_type        VARCHAR(20) NOT NULL CHECK (xref_type IN ('Second-Source','Pin-Compatible','Upgrade','Replacement')),
    source           VARCHAR(30) NOT NULL DEFAULT 'Multi-source',
    notes            VARCHAR(200)
);

-- ---------------------------------------------------------------------
-- rma_cases  (WRITE TARGET for open_rma_agent; 1 pre-seeded)
-- ---------------------------------------------------------------------
CREATE TABLE rma_cases (
    rma_id             VARCHAR(20) PRIMARY KEY,        -- RMA-2026-NNNN
    customer_id        VARCHAR(20) NOT NULL REFERENCES customers(customer_id),
    order_id           VARCHAR(20),
    part_number        VARCHAR(40) NOT NULL,
    quantity           INTEGER     NOT NULL,
    defect_type        VARCHAR(40) NOT NULL,           -- ESD Failure / Field Return / Solderability / Parametric / Mechanical
    description        TEXT,
    status             VARCHAR(15) NOT NULL DEFAULT 'Open' CHECK (status IN ('Open','Under Review','Approved','Rejected','Closed')),
    opened_date        DATE        NOT NULL DEFAULT CURRENT_DATE,
    est_resolution_date DATE
);

-- ---------------------------------------------------------------------
-- sample_requests  (WRITE TARGET for request_sample_agent; 1 pre-seeded)
-- ---------------------------------------------------------------------
CREATE TABLE sample_requests (
    sample_id      VARCHAR(20) PRIMARY KEY,            -- SMP-2026-NNNN
    customer_id    VARCHAR(20) NOT NULL REFERENCES customers(customer_id),
    part_number    VARCHAR(40) NOT NULL,
    quantity       INTEGER     NOT NULL,
    purpose        VARCHAR(160),
    status         VARCHAR(15) NOT NULL DEFAULT 'Requested' CHECK (status IN ('Requested','Approved','Shipped')),
    requested_date DATE        NOT NULL DEFAULT CURRENT_DATE,
    ship_to        VARCHAR(200)
);

-- ---------------------------------------------------------------------
-- alert_subscriptions  (WRITE TARGET for subscribe_alert_agent; starts empty)
-- ---------------------------------------------------------------------
CREATE TABLE alert_subscriptions (
    subscription_id SERIAL PRIMARY KEY,
    customer_id     VARCHAR(20) NOT NULL REFERENCES customers(customer_id),
    part_number     VARCHAR(40) NOT NULL,
    alert_type      VARCHAR(10) NOT NULL CHECK (alert_type IN ('STOCK','PCN')),
    email           VARCHAR(160),
    status          VARCHAR(10) NOT NULL DEFAULT 'Active' CHECK (status IN ('Active','Cancelled')),
    created_date    DATE        NOT NULL DEFAULT CURRENT_DATE
);

-- Indexes on the identifier + foreign keys
CREATE INDEX idx_inventory_part      ON inventory(part_number);
CREATE INDEX idx_pricebreaks_part    ON price_breaks(part_number);
CREATE INDEX idx_orders_customer     ON orders(customer_id);
CREATE INDEX idx_orderlines_order    ON order_lines(order_id);
CREATE INDEX idx_orderlines_part     ON order_lines(part_number);
CREATE INDEX idx_pcn_part            ON pcn_notices(part_number);
CREATE INDEX idx_xref_part           ON cross_references(part_number);
CREATE INDEX idx_rma_customer        ON rma_cases(customer_id);
CREATE INDEX idx_sample_customer     ON sample_requests(customer_id);
CREATE INDEX idx_subs_customer       ON alert_subscriptions(customer_id);
...
-- price_breaks (quantity tiers, USD) --------------------------------
...
-- rma_cases (1 pre-seeded write-target row) --------------------------
...
-- sample_requests (1 pre-seeded write-target row) --------------------
...
-- alert_subscriptions: intentionally EMPTY (filled by subscribe_alert_agent)

-- =====================================================================
-- Row-count sanity (expected):
--   customers 7 | products 10 | inventory 14 | price_breaks 32
--   orders 4 | order_lines 7 | pcn_notices 3 | cross_references 5
--   rma_cases 1 | sample_requests 1 | alert_subscriptions 0
-- =====================================================================
