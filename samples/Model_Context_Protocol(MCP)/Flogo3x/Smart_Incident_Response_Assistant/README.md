# <img width="25" height="25" alt="mcp" src="https://github.com/user-attachments/assets/80bf0bb2-d116-404a-91a0-5b4f3af2e476" /> TIBCO Flogo® 3 Model Context Protocol(MCP) Showcase Sample — Smart Incident Response Assistant

## Overview

This sample demonstrates three advanced MCP features of the **TIBCO Flogo® 3 Connector for Model Context Protocol (MCP)** working together in a single, realistic workflow: **MCP Elicitation**, **MCP Logging**, and **MCP Sampling**.
...
1. Open this sample folder in VS Code with the TIBCO Flogo® 3 extension. It detects the Flogo 3 project(s) — each folder containing `app.fgmd`: `Smart_Incident_Response_Assistant_MCPServer`.
2. Open the app's `.fgprops` file and set the App Properties for your environment (see [App Properties](#app-properties) below) — MCP server port, any LLM/sampling config.
...
4. In **FLOGO3: WORKSPACE APPS EXPLORER**, select the app module → right-click → **Run As Executable**. The native binary is produced in the app's `bin/` folder and logs stream to the integrated terminal. This starts the Flogo MCP Server over HTTP at `http://localhost:9092/mcp`.
5. Configure the MCP endpoint URL with your MCP client and run an incident triage to see Elicitation, Logging, and Sampling in action — see [Test the MCP Server](#test-the-mcp-server) below. Your MCP client must support these MCP features.

## Understanding the configuration

The `Smart_Incident_Response_Assistant_MCPServer` app is a Flogo MCP server (HTTP) that exposes the `incident_response` tool to AI agents. When the tool is called, the flow runs the following sequence:
...
- With the app running, the Flogo MCP Server is available over HTTP at `http://localhost:9092/mcp`.
