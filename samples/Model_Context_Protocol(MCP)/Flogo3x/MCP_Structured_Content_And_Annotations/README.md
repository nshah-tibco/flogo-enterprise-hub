# <img width="25" height="25" alt="mcp" src="https://github.com/user-attachments/assets/80bf0bb2-d116-404a-91a0-5b4f3af2e476" /> TIBCO Flogo® 3 MCP Structured Content & Annotations — E-Commerce Orders Sample

## Overview

This sample demonstrates how to configure **Structured Content** (Tool Output Schema + `structuredContent` mapping) and **Content Annotations** (`audience` + `priority`) in a TIBCO Flogo® 3 MCP Server trigger using a realistic **e-commerce order management** scenario.
...
> **Note:** Tool Output Schema (`reply.outputSchema`), `structuredContent` output mapping, and content annotations (`audience`/`priority` on `response`) are MCP connector features in TIBCO Flogo® 3.
...
The `ECommerceOrdersMCPServer` app exposes **three tools**, each demonstrating a different combination of structured content and annotations:
...
Set these in the app's `.fgprops` file before running (see **Setup & Run** step 2).
...
1. Open this sample folder in VS Code with the TIBCO Flogo® 3 extension. It detects the Flogo 3 project(s) — each folder containing `app.fgmd`: `ECommerceOrdersMCPServer`.
2. Open the app's `.fgprops` file and set the App Properties for your environment (see App Properties above) — MCP server port.
...
5. Once running, the MCP Server is available at the endpoint below. Point your MCP client at it and try the example prompts to see `structuredContent` and `audience`/`priority` annotations in action.
