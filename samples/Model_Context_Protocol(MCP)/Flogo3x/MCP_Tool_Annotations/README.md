# <img width="25" height="25" alt="mcp" src="https://github.com/user-attachments/assets/80bf0bb2-d116-404a-91a0-5b4f3af2e476" /> TIBCO Flogo® 3 MCP Tool Annotations — Banking Operations Sample

## Overview

This sample demonstrates how to configure all four **MCP Tool Annotation hints** in a **TIBCO Flogo® 3 MCP Server** trigger using a realistic **banking operations** scenario.
...
The `BankingOperationsMCPServer` app exposes **six banking tools**, each representing a different annotation combination:
...
In Flogo the annotation hints are configured directly in the **handler settings** of the MCP server trigger. Here is an example from the `BankingOperationsMCPServer` app:
...
1. Open this sample folder in VS Code with the TIBCO Flogo® 3 extension. It detects the Flogo 3 project(s) — each folder containing `app.fgmd`: `BankingOperationsMCPServer`.
2. Open the app's `.fgprops` file and set the App Properties for your environment (see [App Properties](#app-properties) below) — MCP server port.
...
4. In **FLOGO3: WORKSPACE APPS EXPLORER**, select the `BankingOperationsMCPServer` app module → right-click → **Run As Executable**. The native binary is produced in the app's `bin/` folder and logs stream to the integrated terminal.
5. Once running, the Flogo MCP Server is available over HTTP at the MCP endpoint below. Connect your MCP client to it and try the example banking queries to see how the annotation hints guide AI client behavior.
