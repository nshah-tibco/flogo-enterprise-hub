# <img width="25" height="25" alt="mcp" src="https://github.com/user-attachments/assets/80bf0bb2-d116-404a-91a0-5b4f3af2e476" /> TIBCO Flogo® 3 MCP Stateless Server — Product Catalog Sample
...
The `ProductCatalogMCPServer` app exposes four product catalog tools, all read-only and stateless:
...
1. Open this sample folder in VS Code with the TIBCO Flogo® 3 extension. It detects the Flogo 3 project(s) — each folder containing `app.fgmd`: `ProductCatalogMCPServer`.
2. Open the app's `.fgprops` file and set the App Properties for your environment (see App Properties below) — MCP server port.
...
5. Verify and invoke the server using the details below.
