# <img width="25" height="25" alt="mcp" src="https://github.com/user-attachments/assets/80bf0bb2-d116-404a-91a0-5b4f3af2e476" /> TIBCO Flogo® 3 MCP JWT Scope-Based Access Control — Healthcare Patient Records Sample

## Overview

This sample demonstrates how to configure **JWT scope-based access control** and use **enhanced tokenInfo claims** (iss, sub, aud, name, email) in a TIBCO Flogo® 3 MCP Server trigger using a realistic **healthcare patient records** scenario.
...
> **Note:** The `scope` handler setting and enhanced `tokenInfo` claims (iss, sub, aud, name, email) are MCP connector features available in TIBCO Flogo® 3.
...
The `HealthcarePatientRecordsMCPServer` app exposes **four tools**, each protected by a different scope and demonstrating different tokenInfo usage patterns:
...
1. Open this sample folder in VS Code with the TIBCO Flogo® 3 extension. It detects the Flogo 3 project(s) — each folder containing `app.fgmd`: `HealthcarePatientRecordsMCPServer`.
2. Open the app's `.fgprops` file and set the App Properties for your environment (see App Properties below) — JWT secret/issuer/scopes, MCP server port.
...
5. Once running, the MCP Server is available at `http://localhost:9096/mcp`. Generate a JWT with the scopes you want (see **JWT Token Setup**), point your MCP client at the endpoint with an `Authorization: Bearer <token>` header (see **Configure Your MCP Client** and the `claude_desktop_config.json` snippet), then try the example queries in **Try It — Example Prompts** to observe scope enforcement and tokenInfo audit logging.

### JWT Token Setup

The server requires a JWT token with scopes in the `scp` or `scope` claim. You can generate a test token at [jwt.io](https://jwt.io), signed with the value configured in the `FlogoMcpServer.JWT_TOKEN_SECRET` App Property.
...
Set these in the app's `.fgprops` file (`HealthcarePatientRecordsMCPServer/app.fgprops`) before running:

| Property | Default | Description |
|---|---|---|
| `FlogoMcpServer.PORT` | `9096` | HTTP port the MCP server listens on |
| `FlogoMcpServer.JWT_TOKEN_SECRET` | *(pre-configured encrypted secret)* | JWT signing secret used to validate incoming Bearer tokens. Set this to your own secret and sign your test JWTs with the same value |
