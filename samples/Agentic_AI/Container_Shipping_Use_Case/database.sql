-- =============================================================================
-- Maritime Container Shipping — Customer Self-Service AI Assistant
-- PostgreSQL 14+   |   Database: container_shipping
--
-- Schema + engineered demo data for the Agentic AI use case (MCP reads +
-- A2A writes + WebSocket orchestrator). Volatile dates are CURRENT_DATE-relative
-- so the demo always looks current. Load with:
--   psql -h <host> -p <port> -U <user> -d container_shipping -f database.sql
-- =============================================================================

-- ---- Drop in reverse-dependency order --------------------------------------
DROP TABLE IF EXISTS charge_disputes    CASCADE;
DROP TABLE IF EXISTS booking_amendments CASCADE;
...
-- Customers (BCOs / freight customers). Keyed by customer_code — the identifier
-- the user gives in chat.
CREATE TABLE customers (
    customer_code  VARCHAR(20)  PRIMARY KEY,
    company_name   VARCHAR(120) NOT NULL,
    account_type   VARCHAR(20)  NOT NULL CHECK (account_type IN ('BCO','Forwarder','Enterprise')),
    loyalty_tier   VARCHAR(10)  NOT NULL CHECK (loyalty_tier IN ('Gold','Silver','Bronze')),
...
-- Ports (UN/LOCODE).
CREATE TABLE ports (
    port_code  VARCHAR(6)   PRIMARY KEY,
...
CREATE TABLE vessels (
    ...
    vessel_name   VARCHAR(80)  NOT NULL,
...
-- Container types (rate-card dimension).
CREATE TABLE container_types (
    type_code       VARCHAR(6)   PRIMARY KEY,
    description     VARCHAR(60)  NOT NULL,
    teu             NUMERIC(4,1) NOT NULL,
...
CREATE TABLE voyages (
    ...
    origin_port       VARCHAR(6)  REFERENCES ports(port_code),
    destination_port  VARCHAR(6)  REFERENCES ports(port_code),
    etd               DATE,
    eta               DATE,
    transit_days      INTEGER,
    status            VARCHAR(20) CHECK (status IN ('Scheduled','Departed','Arrived','Delayed','Cancelled'))
);

-- Rate card (quote lookup).
CREATE TABLE rates (
    rate_id           VARCHAR(20)   PRIMARY KEY,
    origin_port       VARCHAR(6)    REFERENCES ports(port_code),
    destination_port  VARCHAR(6)    REFERENCES ports(port_code),
    container_type    VARCHAR(6)    REFERENCES container_types(type_code),
...
-- Booking references are auto-generated for agent-created bookings.
CREATE SEQUENCE booking_seq START 100;

-- Bookings. Pre-seeded rows use explicit refs; the book_shipment agent inserts
-- rows that take the generated default (BKG-<year>-01NN).
CREATE TABLE bookings (
    booking_ref       VARCHAR(20)   PRIMARY KEY
                      DEFAULT ('BKG-' || to_char(CURRENT_DATE,'YYYY') || '-' || LPAD(nextval('booking_seq')::text, 4, '0')),
    customer_code     VARCHAR(20)   NOT NULL REFERENCES customers(customer_code),
    voyage_id         VARCHAR(20)   REFERENCES voyages(voyage_id),
    origin_port       VARCHAR(6)    REFERENCES ports(port_code),
    destination_port  VARCHAR(6)    REFERENCES ports(port_code),
    container_type    VARCHAR(6)    REFERENCES container_types(type_code),
    quantity          INTEGER       NOT NULL DEFAULT 1,
    cargo_description VARCHAR(120),
    total_amount_usd  NUMERIC(10,2),
    currency          VARCHAR(3)    NOT NULL DEFAULT 'USD',
    status            VARCHAR(25)   NOT NULL DEFAULT 'Confirmed'
                      CHECK (status IN ('Confirmed','In Transit','Delayed','Discharged','Delivered','Amendment Requested','Cancelled')),
    booking_date      DATE          NOT NULL DEFAULT CURRENT_DATE
);

-- Containers on a booking.
CREATE TABLE containers (
    container_no     VARCHAR(15)  PRIMARY KEY,
    booking_ref      VARCHAR(20)  REFERENCES bookings(booking_ref),
    container_type   VARCHAR(6)   REFERENCES container_types(type_code),
    seal_no          VARCHAR(20),
    status           VARCHAR(20)  CHECK (status IN ('Empty','Loaded','In Transit','Discharged','Delivered','Hold')),
    current_location VARCHAR(6)   REFERENCES ports(port_code),
    gross_weight_kg  INTEGER
);

-- Scan-by-scan movement history.
CREATE TABLE tracking_events (
    event_id      SERIAL       PRIMARY KEY,
    container_no  VARCHAR(15)  REFERENCES containers(container_no),
    event_time    TIMESTAMP    NOT NULL,
    location_port VARCHAR(6)   REFERENCES ports(port_code),
    event_type    VARCHAR(30)  NOT NULL,
    description   VARCHAR(160)
);

-- Charges (freight + accessorials).
CREATE TABLE charges (
    charge_id    VARCHAR(20)   PRIMARY KEY,
    booking_ref  VARCHAR(20)   REFERENCES bookings(booking_ref),
    charge_type  VARCHAR(20)   NOT NULL CHECK (charge_type IN ('Freight','Demurrage','Detention','BAF','THC')),
    amount_usd   NUMERIC(10,2) NOT NULL,
    currency     VARCHAR(3)    NOT NULL DEFAULT 'USD',
    free_days    INTEGER       DEFAULT 0,
    days_used    INTEGER       DEFAULT 0,
    status       VARCHAR(15)   NOT NULL CHECK (status IN ('Invoiced','Paid','Disputed','Waived')),
    charge_date  DATE          NOT NULL DEFAULT CURRENT_DATE
);

-- ---- Agent-written tables ---------------------------------------------------

-- Cargo claims (file_claim_agent inserts; one row pre-seeded so GetClaims shows data).
CREATE TABLE claims (
    claim_id         SERIAL       PRIMARY KEY,
    booking_ref      VARCHAR(20)  NOT NULL REFERENCES bookings(booking_ref),
    customer_code    VARCHAR(20)  NOT NULL REFERENCES customers(customer_code),
    claim_type       VARCHAR(20)  NOT NULL CHECK (claim_type IN ('Damage','Loss','Shortage')),
    description      VARCHAR(240),
    claim_amount_usd NUMERIC(10,2),
    currency         VARCHAR(3)   NOT NULL DEFAULT 'USD',
...
-- Booking amendment requests (amend_booking_agent inserts). Starts empty.
...
-- Charge disputes (dispute_charge_agent inserts). Starts empty.
CREATE TABLE charge_disputes (
    dispute_id           SERIAL       PRIMARY KEY,
    charge_id            VARCHAR(20)  NOT NULL REFERENCES charges(charge_id),
    booking_ref          VARCHAR(20)  REFERENCES bookings(booking_ref),
    reason               VARCHAR(240),
    disputed_amount_usd  NUMERIC(10,2),
    currency             VARCHAR(3)   NOT NULL DEFAULT 'USD',
...
-- ---- Indexes ---------------------------------------------------------------
CREATE INDEX idx_bookings_customer   ON bookings(customer_code);
CREATE INDEX idx_bookings_voyage     ON bookings(voyage_id);
CREATE INDEX idx_containers_booking  ON containers(booking_ref);
CREATE INDEX idx_tracking_container  ON tracking_events(container_no);
CREATE INDEX idx_charges_booking     ON charges(booking_ref);
CREATE INDEX idx_claims_booking      ON claims(booking_ref);
CREATE INDEX idx_rates_lane          ON rates(origin_port, destination_port, container_type);

-- ===========================================================================
-- DEMO DATA
-- ===========================================================================

-- Customers (flagship = CUST-SHIP-1001, Pacific Traders Inc, Gold) ------------
...
-- Tracking events ------------------------------------------------------------
-- MSKU1000001 (BKG-0001, discharged — supports a damage claim)
...
-- OOLU2000001 (BKG-0002, delivered — has a pre-seeded shortage claim)
...
-- HLBU4000001 (BKG-0004, in transit)
...
-- MSKU6000001 (BKG-0006, in transit)
...
-- MSKU7000001 (BKG-0007, FLAGSHIP: weather-held / delayed at transshipment)
...
-- Claims (one pre-seeded; file_claim_agent inserts more) ----------------------
...
-- booking_amendments and charge_disputes start EMPTY (agent-written).

-- ---- Sanity counts (run manually) ------------------------------------------
...
-- SELECT 'charges', count(*) FROM charges UNION ALL
