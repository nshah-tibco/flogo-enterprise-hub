...
- A **chatbot / WebSocket UI** — the shared one under `samples/Agentic_AI/Chatbot`, or any WS test client.
...
6. **Chatbot / WebSocket client.**
   - Point the shared UI (`samples/Agentic_AI/Chatbot`) or a WS test client at
     `ws://<host>:<wsPort>/<path>`.
