...
Set these in the app's `MobileCustomerCareHub/app.fgprops` file (Flogo 3 App Properties) before running. Defaults shown are the values shipped in the sample.

### LLM Provider Connection

| Property | Value |
|---|---|
| `AgenticAI.openai-Mobile.LLM_Provider` | `OpenAI` (change to `Anthropic`, `Google`, etc. for a different provider) |
| `AgenticAI.openai-Mobile.API_Key` | `sk-your-key-here` — set your own API key |
| `AgenticAI.openai-Mobile.LLM_Base_URL` | blank (set for Ollama/vLLM or a custom endpoint) |

### CustomerSupportDispatcher (AIAgent Activity — dispatcher)

| Property | Value |
|---|---|
| `CustomerSupportDispatcher.Activity_Settings.Agents_Handoff` | `BillingSpecialistAgent,TechnicalSupportAgent,UpgradeAdvisorAgent` |
| `CustomerSupportDispatcher.Activity_Settings.LLM_Model` | `gpt-4o` |
| `CustomerSupportDispatcher.Activity_Settings.Token_Limit` | `4096` |
| `CustomerSupportDispatcher.Activity_Settings.Rate_Limit` | `30` |
| `CustomerSupportDispatcher.Activity_Settings.Redact_Sensitive_Data` | `false` |

### BillingSpecialistAgent (Agent Trigger — port 8081)

| Property | Value |
|---|---|
| `BillingSpecialistAgent.Trigger_Settings.AgentName` | `BillingSpecialistAgent` |
| `BillingSpecialistAgent.Trigger_Settings.AgentDescription` | Mobile billing specialist description |
| `BillingSpecialistAgent.Trigger_Settings.LLM_Model` | `gpt-4o` |
| `BillingSpecialistAgent.Trigger_Settings.LLM_temperature` | `1` |
| `BillingSpecialistAgent.Trigger_Settings.Token_Limit` | `4096` |
| `BillingSpecialistAgent.Trigger_Settings.Rate_Limit` | `30` |
| `BillingSpecialistAgent.Trigger_Settings.Redact_Sensitive_Data` | `true` (PII redaction enabled) |

### TechnicalSupportAgent (Agent Trigger — port 8082)

| Property | Value |
|---|---|
| `TechnicalSupportAgent.Trigger_Settings.AgentName` | `TechnicalSupportAgent` |
| `TechnicalSupportAgent.Trigger_Settings.AgentDescription` | Technical support specialist description (can hand off to UpgradeAdvisorAgent) |
| `TechnicalSupportAgent.Trigger_Settings.LLM_Model` | `gpt-4o` |
| `TechnicalSupportAgent.Trigger_Settings.LLM_temperature` | `1` |
| `TechnicalSupportAgent.Trigger_Settings.Token_Limit` | `4096` |
| `TechnicalSupportAgent.Trigger_Settings.Rate_Limit` | `30` |
| `TechnicalSupportAgent.Trigger_Settings.Redact_Sensitive_Data` | `false` |

### UpgradeAdvisorAgent (Agent Trigger — port 8083)

| Property | Value |
|---|---|
| `UpgradeAdvisorAgent.Trigger_Settings.AgentName` | `UpgradeAdvisorAgent` |
| `UpgradeAdvisorAgent.Trigger_Settings.AgentDescription` | Upgrade advisor description (receives handoffs from Dispatcher and TechnicalSupportAgent) |
| `UpgradeAdvisorAgent.Trigger_Settings.LLM_Model` | `gpt-4o` |
| `UpgradeAdvisorAgent.Trigger_Settings.LLM_temperature` | `0.6` |
| `UpgradeAdvisorAgent.Trigger_Settings.Token_Limit` | `4096` |
| `UpgradeAdvisorAgent.Trigger_Settings.Rate_Limit` | `30` |
| `UpgradeAdvisorAgent.Trigger_Settings.Redact_Sensitive_Data` | `false` |

---

## Setup & Run

1. Open this sample folder in VS Code with the TIBCO Flogo® 3 extension. It detects the Flogo 3 project(s) — each folder containing `app.fgmd`: `MobileCustomerCareHub`.
2. Open the app's `.fgprops` file and set the App Properties for your environment (see App Properties below) — LLM provider/API key, ports.
...
4. In **FLOGO3: WORKSPACE APPS EXPLORER**, select the app module → right-click → **Run As Executable**. The native binary is produced in the app's `bin/` folder and logs stream to the integrated terminal. The app starts a WebSocket server on port **9998**, and the three specialist Agent Triggers come up on ports **8081** (Billing), **8082** (Technical), and **8083** (Upgrade).
5. Invoke and verify the two entry endpoints:

   ### Test — AI-Driven Routing (non-deterministic)
   ...
   **Multi-domain scenario (watch the multi-hop handoff):**
   ...
   ### Test — Deterministic Direct Billing
