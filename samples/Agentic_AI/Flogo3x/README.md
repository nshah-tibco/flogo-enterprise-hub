# TIBCO Flogo® 3 Agentic AI Connector Samples

This directory contains **real-world sample applications** demonstrating the full capabilities of the **TIBCO Flogo® 3 Agentic AI Connector** — the enterprise-grade way to build, orchestrate, and govern AI agents inside Flogo integration flows.

> These are the **Flogo 3** versions of the Agentic AI samples. Each sample is a folder-based Flogo 3 project (a folder containing `app.fgmd`, `app.fgprops`, `flows/`, `triggers/`, and `connections/`) rather than a single `.flogo` file.
...
- **TIBCO Flogo® 3.0.0 or later** with the TIBCO Flogo® 3 VS Code extension. For more information, please refer to the [documentation](https://docs.tibco.com/pub/flogo/latest/doc/html/Default.htm#connectors/agentic-AI/agentic-AI-overview.htm)
- An API key for your chosen LLM provider (OpenAI, Gemini, or Anthropic)
- A WebSocket client for testing: [Flogo Chatbot](../Chatbot/) (included in the 2.x sample directory — see below) or [websocat](https://github.com/vi/websocat)

## Quick Start

1. Clone or download this repository.
2. Open a sample folder under `samples/Agentic_AI/Flogo3x/<sample-name>/` in VS Code with the TIBCO Flogo® 3 extension. The extension detects each Flogo 3 project (a folder containing `app.fgmd`).
3. Open each app's `.fgprops` file and set the App Properties for your environment (LLM provider, model, API key, ports, backend URLs).
...
6. Connect via the [Flogo Chatbot](../Chatbot/) or websocat to test any sample that exposes a WebSocket endpoint.

### Flogo Chatbot — Browser-Based WebSocket Test Client

A ready-to-use chat UI is included in the 2.x sample directory at [`../Chatbot/`](../Chatbot/) for testing any sample that exposes a WebSocket endpoint. It is a plain Node.js web app (not a Flogo project), so it is shared as-is by both the 2.x and 3.x samples:
