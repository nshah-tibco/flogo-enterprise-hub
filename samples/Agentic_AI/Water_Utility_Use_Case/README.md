...
- Chatbot UI (from `samples/Agentic_AI/Chatbot/`)
...
Once the three apps are generated (see the note at the top of this README), they carry placeholders /
reference-app values for every secret; replace them with your own before an end-to-end run. Never commit
...
4. **Ports free & consistent.**
   - MCP **9782**, A2A **9783–9786**, and the orchestrator WebSocket **9780** must all be free on the host.
   - The orchestrator's MCP `serverUrl` (`http://localhost:9782/water-cis`) and the four A2A
     `serverUrl`s (9783–9786) must match those ports. If you change a port, change it in the app
     property **and** in the orchestrator connection URL.

5. **Chatbot / WebSocket client.**
   - The orchestrator exposes `ws://<host>:9780/water`. Point the shared Chatbot UI
     (`samples/Agentic_AI/Chatbot/`) at it — see `prompts.md` for ready-to-paste demo prompts.
...
- [ ] DB `water_utility` created, `database.sql` loaded, `reset_data.sql` run; row counts sane (customers 18, invoices 18, tariff_plans 4)
...
- [ ] All ports free (9780, 9782, 9783–9786); orchestrator MCP/A2A URLs match the MCP/A2A ports
...
- [ ] Start order: MCP (9782) → A2A (9783–9786) → Orchestrator (9780); each logs a clean start
- [ ] Chatbot UI connects to `ws://localhost:9780/water` and gets a reply
